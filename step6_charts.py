...
runs_df = pd.read_pickle('runs_df.pkl')
...
ax.legend()
fig.tight_layout()
fig.savefig('chart1_runlength.png', dpi=170)
plt.close(fig)

# ---- Chart 2: P(win|prev win) vs P(win|prev loss) by discipline ----
lag_df = pd.read_pickle('lag_df.pkl')
disc_stats = []
...
ax.legend()
fig.tight_layout()
fig.savefig('chart2_discipline.png', dpi=170)
plt.close(fig)

# ---- Chart 3: permutation null distribution vs observed ----
null_stats = np.load('null_stats.npy')
fig, ax = plt.subplots(figsize=(6.5, 4))
...
ax.legend()
fig.tight_layout()
fig.savefig('chart3_permutation.png', dpi=170)
plt.close(fig)
