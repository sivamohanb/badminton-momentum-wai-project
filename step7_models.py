...
pdf = pd.read_pickle('point_level.pkl').dropna(subset=['lag2']).reset_index(drop=True)
...
for name, model in models.items():
    ...
    fig.savefig(f'cm_{safe}.png', dpi=170)
    plt.close(fig)
...
fig_roc.savefig('roc_all_models.png', dpi=170)
plt.close(fig_roc)
...
fig.tight_layout()
fig.savefig('rf_feature_importance.png', dpi=170)
plt.close(fig)
